...
export default function UpdateIncomePage() {
  ...
  const [detailIncomes, setDetailIncomes] = useState<DetailIncome[]>([]);
  ...
  const fetchDetailIncomesById = async (id: string) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/detail-input/input/${id}`
      );
      const data: DetailIncome[] = response.data.data;
      setDetailIncomes(data);
      setDetailIncomesTemp(data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const fetchIncome = async (id: string) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/input/${id}`);
      const data: Income = response.data.data;
      setIncome(data);
      setNoInput(data.no_input);
      setDateInput(data.date_input);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:8080/api/detail-inputs/${masterIncome.ID}`,
        detailIncomes
      );
      if (res.data) {
                  ...
                  <tr>
                    ...
                    <td className="border py-2 px-4">
                      <input
                        ...
                        defaultValue={detailIncome.input_information}
                        onChange={(e) => {
                    ...
                    <td className="border py-2 px-4">
                      <input
                        type="number"
                        className="border rounded px-2 py-1 grow bg-slate-100 w-full text-center"
                        defaultValue={detailIncome.quantity}
                        onChange={(e) => {
                    ...
                    <td className="border py-2 px-4">
                      <input
                        type="number"
                        className="border rounded px-2 py-1 grow bg-slate-100 w-full text-center"
                        defaultValue={detailIncome.total_price}
                        onChange={(e) => {
